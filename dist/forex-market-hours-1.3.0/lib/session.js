...
export function computeMarketStatus(market, now = new Date(), opts = {}) {
  ...
  const countdownInterval = currentTrading || nextTrading;
  const closesIn = currentTrading ? currentTrading.close - nowMs : null;
  const opensIn = countdownInterval ? countdownInterval.open - nowMs : null;

  // Any market: within 1h of the next regular open → Pre-market (FX or Cash mode).
