...
/**
 * Whether the global spot-Forex market is open (America/New_York clock).
 * Closed: all Saturday; Sunday before 17:00; Friday from 17:00 onward.
 * @param {Date} [now]
 */
export function isForexSpotOpenAt(now = new Date()) {
  const p = getZonedParts(now, 'America/New_York');
  const mins = p.hour * 60 + p.minute;
  if (p.weekday === 6) return false;
  if (p.weekday === 0 && mins < 17 * 60) return false;
  if (p.weekday === 5 && mins >= 17 * 60) return false;
  return true;
}
