...
// a free Finnhub API key in Settings (see lib/finnhub.js).
...
/** @type {Record<string, {date:string, name:string, closeAt?:string}[]>} */
const runtimeHolidaysByMarket = {};

/** Replace Finnhub-fetched holidays for a market (merged with local on read). */
export function setRuntimeHolidays(marketId, holidays) {
  if (!marketId || !Array.isArray(holidays)) return;
  runtimeHolidaysByMarket[marketId] = holidays;
}

export function clearRuntimeHolidays() {
  for (const k of Object.keys(runtimeHolidaysByMarket)) {
    delete runtimeHolidaysByMarket[k];
  }
}

function holidayListFor(market) {
  const local = HOLIDAYS[market.id];
  const api = runtimeHolidaysByMarket[market.id];
  if (!api?.length) return local;
  if (!local?.length) return api;
  const byDate = new Map(local.map(h => [h.date, h]));
  for (const h of api) byDate.set(h.date, h);
...
export function getHoliday(market, dateStr) {
  const list = holidayListFor(market);
  if (!list) return null;
